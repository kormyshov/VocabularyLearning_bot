...
class RequestToAddSampleAction(AbstractAction):
    ...
    def check(self, user: User, text: str) -> bool:
        return user.is_request_to_add_definition() and text != BACK[user.language]

    @logger
    def do(self, viewer: AbstractViewer, user: User, text: str) -> None:
        definition_id = user.add_definition(text)
        user.request_to_add_sample(definition_id)
        viewer.view(user.id, ENTER_SAMPLE[user.language], get_back_or_without_sample(user.language))
